...
class SpeechService:
    # Статические переменные для моделей
    _model_ru: Optional[Model] = None
    _whisper_model = None
    
    # Вопросительные слова и фразы для русского языка
    QUESTION_WORDS_RU = {
    ...
    def __init__(self):
        # Предварительно загружаем модели при создании сервиса
        self.get_model_ru()
    ...
    def is_question(self, text: str) -> bool:
        ...
        # Если первое слово вопросительное
        if first_word in self.QUESTION_WORDS_RU:
            return True
        
        # Если есть вопросительное слово в середине предложения
        if any(word in self.QUESTION_WORDS_RU for word in words[1:]):
            return True
        
        # Проверяем интонационные вопросы
        intonation_patterns = [
            r'^[а-я\s]+\?$',  # Заканчивается на ?
            r'.*\bли\b.*',    # Содержит частицу "ли"
            r'^а\b.*',        # Начинается с "а"
            r'.*\bправда\b.*',  # Содержит слово "правда"
            r'.*\bда\b\s*$',    # Заканчивается на "да"
            r'.*\bтак\b\s*$',   # Заканчивается на "так"
            r'.*\bверно\b\s*$', # Заканчивается на "верно"
            r'.*\bразве\b.*',   # Содержит "разве"
            r'.*\bнеужели\b.*', # Содержит "неужели"
            r'\bты\b.*',        # Начинается с личных местоимений
            r'\bвы\b.*',
            r'\bон\b.*',
            r'\bона\b.*',
            r'\bоно\b.*',
        ...
        # Если соответствует любому из паттернов
    ...
    def add_punctuation(self, text: str, is_russian: bool = True) -> str:
        ...
        if is_russian:
            # Для русского языка
            # Проверяем на вопрос до добавления знаков препинания
            if self.is_question(text):
                return text + "?"
            
            # Проверяем на восклицательные слова/фразы
    ...
    def recognize_speech(self, audio_path: str, is_russian: bool = True) -> str:
        ...
        if is_russian:
            ...
            # Используем Whisper для тайского
